 ...
 */
@Slf4j
class MavenExecutableInstall {

    static installPath = System.getProperty("user.home") + File.separator + ".mvnx"


    public static void install(MavenExecutableParams params) {
        ...
        if (params.type == 'maven-plugin') {
            String coords = MavenExecutable.makeCoords(targetArtifact)
            command = 'mvn ' + coords + ':' // then immediately goal name as splat arg
        }
        else { // type == 'jar'
            command = MavenExecutable.buildCommandString(params) + ' ' // extra space for splat args
        }
